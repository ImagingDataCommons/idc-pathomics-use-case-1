...
def per_slide_evaluation(model, data_set):
    ...
    for data_point in data_set.data_points:
        ...
        #prediction = model.predict(data_point) # frage: kommt bei n=2 dann der eine Wert auch als np datentyp raus? ist das ev. egal?
        prediction = 1
        slide_results[slide_id]['num_patches'] += 1
        slide_results[slide_id]['predictions'].append(prediction)

    for slide_id in slide_results:
        slide_results[slide_id]['predictions'] = np.array(slide_results[slide_id]['predictions'])
        print(slide_results[slide_id]['predictions'].shape)
